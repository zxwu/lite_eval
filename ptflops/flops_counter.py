...
def profile(model, input_res, print_per_layer_stat=True, as_strings=0,
    ...
    if input_constructor:
        input, input_small = input_res
        output, r_stack = flops_model(input, input_small)
    else:
    ...
    return flops_count, params_count, output, r_stack
...
def lstm_flops_counter_hook(module, input, output):
    input = input[0]
    batch_size = input.shape[0]
    num_output = output[0].shape[1]
    module.__flops__ += int(batch_size * input.shape[1] * num_output + num_output*num_output + num_output)
...
def add_flops_counter_hook_function(module):
    if is_supported_instance(module):
        ...
        if isinstance(module, torch.nn.Conv2d):
            ...
            handle = module.register_forward_hook(lstm_flops_counter_hook)
        else:
